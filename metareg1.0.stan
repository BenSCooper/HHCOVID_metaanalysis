/**
  Meta-regression analysis v.1.0
  Changes from v0.5 is that we added a generated quantities block 
  to output predicted main of daily acuqisition rates and mean for a new trial with different levels of hand hygiene compliance
***/
...
generated quantities {
  ...
  real<lower=0> predicted_mean[21];
  real<lower=0> predicted_mean_new_trial[21];
  ...
  for (i in 1:21){
    ...
    predicted_mean[i] =  exp(a0 + b0*(i-1));
    //now calculate mean predicted prob of infection for a new trial 
    predicted_mean_new_trial[i]  = exp(a_new + b_new*(i-1));
  }  
}  
